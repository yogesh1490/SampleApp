...
export default class UpdateEmployee extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      input_user_id: this.props.navigation.state.params.employeeIdOBJ,
      user_name: '',
      user_contact: '',
      user_address: '',
    };
  ...
  searchUser(){
  //  ToastAndroid.show('searchUser method called', ToastAndroid.SHORT);
              ...
              (tx, results) => {
                ...
                if(results.rowsAffected>0){
                  Alert.alert( 'Success', 'User updated successfully',
                    [
                      {text: 'Ok', onPress: () => that.props.navigation.navigate('NumEmployee')},
                    ],
                    { cancelable: false }
                  );
                }else{
